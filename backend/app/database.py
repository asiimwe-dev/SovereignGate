...
def init_db():
    ...
    cursor.execute("SELECT batch_id FROM payment_batches WHERE batch_id = ?", (batch2_id,))
    if not cursor.fetchone():
        batch2_payload = {
            "amount": 2500000000,
            "currency": "UGX",
    ...
    cursor.execute("SELECT batch_id FROM payment_batches WHERE batch_id = ?", (batch3_id,))
    if not cursor.fetchone():
        batch3_payload = {
            "amount": 7800000000,
            "currency": "UGX",
    ...
    cursor.execute("SELECT batch_id FROM payment_batches WHERE batch_id = ?", (batch4_id,))
    if not cursor.fetchone():
        batch4_payload = {
            "amount": 4200000000,
            "currency": "UGX",
