...
async def inject_malicious_script():
    ...
    try:
        ...
        cursor.execute(
            "SELECT batch_id, payload_json, status FROM payment_batches LIMIT 1"
        )
        row = cursor.fetchone()
        ...
        logger.warning(f"FORENSIC: Original recipient = {original_recipient}")
        ...
        logger.warning(f"FORENSIC: Mutated recipient = {mutated_recipient}")
...
async def reset_system():
    ...
    try:
        ...
        logger.info(
            "SYSTEM RESET COMPLETE: Forensic cleanup finished, normal operation resumed"
        )
