...
async def submit_share(submission: ShareSubmission):
    ...
    try:
        ...
        if not FidoEmulator.validate_token(submission.hardware_token):
            raise HTTPException(
                status_code=401, detail="Invalid hardware token challenge response"
            )
        ...
        share_added = MemoryBuffer.add_share(
            submission.batch_id, submission.admin_id, share_val
        )
...
async def execute_settlement(batch_id: str):
    ...
    try:
        ...
        cursor.execute(
            "SELECT payload_json, status FROM payment_batches WHERE batch_id = ?",
            (batch_id,),
        )
        row = cursor.fetchone()
        ...
        if row["status"] == "CRITICAL_COMPROMISE":
            raise HTTPException(
                status_code=403, detail="System locked due to compromise"
            )
        ...
        logger.info(
            f"Executing settlement: Reconstructing master key from {len(shares)} shares for batch {batch_id}"
        )
