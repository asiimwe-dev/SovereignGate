...
const CommandConsole = () => {
  const { batch, selectedBatch, loading, sharesSubmitted, setSharesSubmitted, fetchStatus, fetchBatches } = useSystem();
  ...
  const handleExecuteClearing = async () => {
    ...
    try {
      ...
      // Immediately refetch DB states to synchronize all components
      await fetchStatus();
      await fetchBatches();
      ...
      <div className="flex flex-col items-center justify-center h-[50vh]">
        <div className="animate-pulse-gold">
          <div className="w-14 h-14 rounded-2xl bg-[#0f131a] border border-[#C5A059]/40 flex items-center justify-center mb-6 shadow-lg">
            <Cpu className="text-[#C5A059] animate-spin" size={28} style={{ animationDuration: '6s' }} />
          </div>
        </div>
        <p className="font-mono text-sm text-slate-400 uppercase tracking-[0.2em] font-bold">
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <section className="institutional-card p-6 md:p-8 bg-[#0f131a] border-[#1f2937]">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 pb-5 border-b border-[#1f2937]">
          <div>
            <h2 className="institutional-header text-2xl md:text-3xl font-black mb-1.5 flex items-center gap-2">
              <span className="text-[#C5A059]">◆</span> {currentBatch.funding_vote}
            </h2>
            <p className="data-mono text-xs text-slate-500 uppercase tracking-widest">{currentBatch.batch_id}</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="institutional-card p-4 bg-[#07090e] border-[#1f2937] hover:border-[#1f2937]">
            <p className="text-xs text-slate-500 uppercase font-black mb-1.5 tracking-widest">Source Account</p>
            <p className="data-mono text-sm text-slate-300 font-bold break-all">{currentBatch.source_account}</p>
          </div>
          <div className="institutional-card p-4 bg-[#07090e] border-[#1f2937] hover:border-[#1f2937]">
            <p className="text-xs text-slate-500 uppercase font-black mb-1.5 tracking-widest">Authorized Payee</p>
            <p className="text-sm font-black text-slate-200 uppercase tracking-tight truncate">{currentBatch.payload?.recipient}</p>
          </div>
          <div className="institutional-card p-4 bg-gradient-to-br from-[#07090e] to-[#0f131a] border-[#1f2937] hover:border-[#1f2937]">
            <p className="text-xs text-[#C5A059] uppercase font-black mb-1 tracking-widest flex items-center gap-1.5">
              <DollarSign size={14} /> Approved Amount
            </p>
      ...
      <section className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="institutional-header text-xl font-black flex items-center gap-2">
              3-Key Authorization Chamber
            </h3>
            <p className="text-xs text-slate-500 uppercase tracking-widest mt-0.5">Volatile memory signature accumulation</p>
          </div>
          <div className="text-sm font-mono text-slate-300 uppercase tracking-widest px-3 py-1.5 bg-[#0f131a] rounded-lg border border-[#1f2937]">
                ...
                <div className="p-5 space-y-4">
                  ...
                  <div className="flex justify-between items-start gap-3">
                    <div className="flex-1">
                      <p className="text-xs font-black text-[#C5A059] uppercase tracking-widest mb-0.5">Node {admin.id}</p>
                      <h4 className="institutional-header text-base font-black leading-snug tracking-tight text-slate-100">{admin.role}</h4>
                      <p className="text-xs text-slate-500 font-mono uppercase tracking-wider mt-1">{admin.name}</p>
                    </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <input 
                        ...
                        className="w-full bg-[#07090e] border border-[#1f2937] rounded-lg px-3.5 py-3 text-sm font-mono text-slate-300 focus:outline-none focus:border-[#C5A059]/40 disabled:opacity-40 disabled:bg-[#07090e] transition-colors"
                        placeholder="••••••••••••••••"
                      />
                      {isSigned && (
                        <div className="absolute inset-0 bg-emerald-950/20 rounded-lg flex items-center justify-center border border-emerald-500/10">
                          <span className="text-xs font-mono text-emerald-400 font-black tracking-[0.2em] uppercase">Committed</span>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    className={`w-full py-3 rounded-lg font-black text-xs uppercase tracking-[0.15em] flex items-center justify-center gap-2 transition-all active:scale-[0.98] cursor-pointer ${
                      isSigned 
      ...
      <section className="space-y-4">
        ...
        <div className="relative">
          ...
          <button
            ...
            className={`w-full py-4 rounded-xl font-black text-base uppercase tracking-[0.2em] relative transition-all active:scale-[0.99] cursor-pointer flex items-center justify-center gap-2 ${
          ...
          >
            {executing ? (
              <>
                <div className="inline-block w-5 h-5 rounded-full border-2 border-black/35 border-t-black animate-spin" />
                Executing Settlement Clearing...
              </>
            ) : isSettled ? (
              <>
                <CheckCircle2 size={18} /> Transaction Settled & Disbursed
              </>
            ) : !thresholdReady ? (
              <>
                <Lock size={18} /> Awaiting Quorum ({signatureCount} of 2 Shares committed)
              </>
            ) : (
              <>
                Execute Settlement Clearing <ArrowRight size={18} />
              </>
            )}
          </button>
        </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 space-y-2">
              ...
              <div className="space-y-1">
                <span className="text-xs font-black text-slate-500 uppercase tracking-widest block">Combined Vault Signature</span>
                <p className="data-mono text-xs text-emerald-400 bg-black/60 p-3.5 rounded-lg border border-emerald-500/10 break-all leading-relaxed">
