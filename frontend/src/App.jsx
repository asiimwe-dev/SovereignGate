  ...
  if (!system) {
    return (
      <div className="min-h-screen bg-[#07090e] flex items-center justify-center">
        <div className="text-red-500 font-mono animate-pulse text-base">
          CRITICAL SYSTEM ERROR: CONTEXT UNAVAILABLE
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen relative flex flex-col md:flex-row transition-all duration-700 bg-[#07090e]">
      ...
      */}
      ...
      <main
        className={`w-full md:w-[75%] p-6 md:p-8 bg-[#07090e] border-r border-[#1f2937] relative selection:bg-[#C5A059]/20 transition-all duration-700 ${isCompromised ? "pointer-events-none opacity-40 blur-[4px]" : ""}`}
      >
        ...
        <header className="mb-8 border-b border-[#1f2937] pb-6 flex justify-between items-start md:items-center flex-col md:flex-row gap-4 relative z-10">
          <div>
            ...
            <p className="text-xs text-slate-400 font-mono uppercase tracking-widest ml-14 mt-1">
              RTGS Settlement Terminal — Multi-Signature Authorization Protocol
            </p>
          </div>
          ...
          <div className="flex items-center gap-5 px-4 py-2.5 bg-[#0f131a] rounded-xl border border-[#1f2937] shadow-sm">
            <div>
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                ...
                <span className="text-sm font-black text-slate-200 uppercase tracking-tight">
                  OPERATIONAL
                </span>
              </div>
            </div>
            ...
            <div>
              ...
              <span className="text-sm font-mono text-emerald-400 font-bold tracking-tight mt-0.5 block">
                99.99997%
              </span>
            </div>
          </div>
      ...
      <aside
        className={`w-full md:w-[25%] bg-[#0f131a] relative border-l border-[#1f2937] md:sticky md:top-0 md:h-screen overflow-y-auto transition-all duration-700 ${isCompromised ? "pointer-events-none opacity-40 blur-[4px]" : ""}`}
      >
        <HeistControl />
      </aside>
    </div>
